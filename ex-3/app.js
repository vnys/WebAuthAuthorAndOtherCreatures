...
app.get('/callback', function (req, res) {
  ...
  if (req.query.state != state) {
    logger.error(
      'State DOES NOT MATCH: expected ' + state + ' got ' + req.query.state,
    )
  ...
  request('POST', authServer.tokenEndpoint, {
    ...
    if (tokenRes.statusCode >= 200 && tokenRes.statusCode < 300) {
      ...
      tokenInfo.exp = moment(payLoad.exp * 1000).format(
      ...
      res.render('index', {
        title: 'Authenticated and access',
        code: code.substr(1, 10),
        tokenInfo: tokenInfo,
      })
    } else {
      logger.debug(
        'We did not get access token ' + tokenRes.statusCode + tokenRes.body,
      )
